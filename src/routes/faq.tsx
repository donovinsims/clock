    ...
    script: [
      {
        ...
        children: JSON.stringify({
          ...
          "@type": "FAQPage",
          mainEntity: FAQ_CATEGORIES.flatMap((c) => c.questions).map((faq) => ({
            "@type": "Question",
            name: faq.q,
            acceptedAnswer: {
              "@type": "Answer",
        ...
        <div className="mx-auto max-w-3xl space-y-16">
          {FAQ_CATEGORIES.map((category) => (
            <div key={category.label}>
              <div className="op-label mb-6">{category.label}</div>
              <div className="space-y-0">
                {category.questions.map((faq, i) => (
                  <details
                    key={i}
                    className="group border-b border-border py-5"
                    {...(category.label === "Trust & Credibility" && i === 0 ? { open: true } : {})}
                  >
                    <summary className="flex cursor-pointer items-start justify-between gap-4 text-base font-display transition-colors hover:text-ink/70">
                      {faq.q}
                      <ChevronDown className="mt-1 h-4 w-4 shrink-0 transition-transform group-open:rotate-180" aria-hidden="true" />
                    </summary>
                    <div className="mt-3 pb-2 text-base leading-relaxed text-ink/70 max-w-2xl">
                      {faq.a}
                    </div>
                  </details>
                ))}
